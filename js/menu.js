...
const COFFEE_HALF_EXTRA = '.coffee-half-cols{display:flex;gap:0.35in;flex:1;overflow:hidden;}.coffee-half-cols>div{flex:1;min-width:0;}.coffee-half-entry{display:flex;flex-direction:column;flex-shrink:0;border-bottom:1px solid rgba(160,130,90,0.3);padding:11px 0;}.coffee-half-row{display:flex;justify-content:space-between;align-items:flex-start;gap:8px;}.coffee-half-name{font-family:\'Oswald\',sans-serif;font-size:16px;font-weight:700;color:#1a1410;text-transform:uppercase;letter-spacing:0.03em;line-height:1.3;flex-shrink:0;}.coffee-half-sizes{display:flex;flex-direction:column;align-items:flex-end;flex-shrink:0;gap:2px;}.coffee-half-size-line{display:flex;gap:8px;font-family:\'Inter\',sans-serif;font-size:13px;font-weight:500;color:#3a3530;line-height:1.5;white-space:nowrap;}.coffee-half-size-price{font-family:\'Oswald\',sans-serif;font-weight:700;color:#1a1410;min-width:40px;text-align:right;}.coffee-half-desc{font-family:\'Inter\',sans-serif;font-size:12.5px;font-style:normal;font-weight:400;color:#4a4440;line-height:1.45;margin-top:4px;}.coffee-half-footer{padding-top:16px;text-align:center;font-family:\'Inter\',sans-serif;flex-shrink:0;}.coffee-half-footer-title{font-family:\'Oswald\',sans-serif;font-size:15px;font-weight:700;letter-spacing:0.1em;color:#8b3a1a;text-transform:uppercase;margin-bottom:7px;}.coffee-half-footer-line{font-size:12.5px;color:#3a3530;margin-bottom:4px;}';
...
async function genCoffeeHalf() {
  ...
  try {
    ...
    rows.forEach((c) => { if (!groups[c.drink_name]) { groups[c.drink_name] = []; order.push(c.drink_name); } groups[c.drink_name].push(c); });
    const mid = Math.ceil(order.length / 2);
    const leftCol = order.slice(0, mid).map((name) => coffeeHalfEntry(name, groups[name])).join('');
    const rightCol = order.slice(mid).map((name) => coffeeHalfEntry(name, groups[name])).join('');
    const footer = '<div class="coffee-half-footer"><div class="coffee-half-footer-title">Make It Yours</div>'
      + '<div class="coffee-half-footer-line">Vanilla · Caramel · Lavender · Raspberry · Hazelnut — flavor +$0.75</div>'
      + '<div class="coffee-half-footer-line">Extra Shot +$1.00 · Oat Milk +$0.75</div></div>';
    const inner = '<div class="coffee-half-cols"><div>' + leftCol + '</div><div>' + rightCol + '</div></div>' + footer;
