...
const COFFEE_HALF_EXTRA = '.half-card{width:5.1in;height:8.1in;padding:0.4in 0.35in;}.coffee-half-cols{display:flex;gap:0.3in;flex:1;}.coffee-half-cols>div{flex:1;min-width:0;}.coffee-half-cols .section-label{margin-top:8px;}.coffee-half-cols .section-label:first-child{margin-top:0;}.coffee-half-cols .beer-desc{font-style:normal;font-size:11px;color:#4a4440;margin-bottom:2px;}.coffee-half-footer{padding-top:14px;text-align:center;font-family:\'Inter\',sans-serif;flex-shrink:0;}.coffee-half-footer-title{font-family:\'Oswald\',sans-serif;font-size:14px;font-weight:700;letter-spacing:0.1em;color:#8b3a1a;text-transform:uppercase;margin-bottom:6px;}.coffee-half-footer-line{font-size:11.5px;color:#3a3530;margin-bottom:3px;}';
function coffeeHalfEntry(name, items) {
  const desc = (items.find((c) => c.description) || {}).description;
  const rows = items.map((c) => '<div class="beer-row"><div class="beer-left"><span class="beer-name">' + escHtml(c.size_label) + '</span></div><div class="beer-right"><span class="beer-price">' + fmtPFull(c.price) + '</span></div></div>').join('');
  return '<div class="section-label">' + escHtml(name) + '</div>' + rows + (desc ? '<div class="beer-desc">' + escHtml(desc) + '</div>' : '');
}
